...
export type FormValidation = {
  [Property in keyof Omit<Vehicle, "id">]: boolean;
};

const defaultFormValues: Partial<Vehicle> = {
  make: "",
  model: "",
  year: "",
  price: "",
...
const defaultFormErrors: FormValidation = {
  make: false,
  model: false,
  year: false,
  price: false,
...
export default function Modal({ open, setOpen, formDataOverride, add }: Props) {
  const [formValues, setFormValues] =
    useState<Partial<Vehicle>>(defaultFormValues);
  const [formErrors, setFormErrors] =
    useState<FormValidation>(defaultFormErrors);
  ...
  function formValidation() {
    ...
    if (!formValues.make) {
      newFormErrors.make = true;
      failedValidation = true;
    }
    if (!formValues.model) {
      newFormErrors.model = true;
      failedValidation = true;
    }
    if (!formValues.price) {
    ...
    if (!formValues.year) {
      newFormErrors.year = true;
      failedValidation = true;
    }
    if (!formValues.status) {
      newFormErrors.status = true;
      failedValidation = true;
    }
